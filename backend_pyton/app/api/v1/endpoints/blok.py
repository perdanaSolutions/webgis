...
from sqlalchemy.orm import Session
from app.api import deps  # Impor file deps kita tadi
from app.models.auth import User # Impor model User jika ingin mencatat log nanti

router = APIRouter()

# 1. API GET Blocks (Hanya boleh diakses oleh yang punya permission "blok:read" atau superadmin)
@router.get("/blocks")
def get_blocks(
    ...
    # KODE QUERY GEOMETRI / BLOK KAMU YANG SUDAH ADA DI SINI...
    return {"status": "success", "accessed_by": current_user.username, "data": []}


# 2. API POST/CREATE Blocks (Hanya boleh diakses oleh yang punya permission "blok:write" atau superadmin)
@router.post("/blocks")
def create_block(
