...
# =====================================================================
# MODUL UPLOAD & FETCH GEO_SLOPE (SPATIAL MULTIPOLYGON SLOPE)
# =====================================================================

def analyze_geojson_slope(db: Session, geojson_content: bytes, bulan: int, tahun: int) -> dict:
    """
    Tahap 1 Slope: Menganalisis validitas poligon kemiringan lereng terhadap master blok
...
def execute_bulk_slope(db: Session, geojson_content: bytes, filename: str, bulan: int, tahun: int, user_id: str = None) -> dict:
    """
    Tahap 2 Slope: Eksekusi pengisian spasial poligon slope secara massal,
        ...
        db.execute(
            text("""
                INSERT INTO sys_upload_log (
                    ...
                    :batch_id, 'GEOJSON_UPLOAD', 'geo_slope', :filename, 
    ...
    db.execute(
        text("DELETE FROM geo_slope WHERE bulan = :b AND tahun = :t"),
        {"b": bulan, "t": tahun}
    )
    ...
    # 4. Looping Insert Poligon Slope
    for index, feature in enumerate(features):
        ...
        try:
            ...
            # Memasukkan data MultiPolygon/Polygon dengan ST_Multi untuk konsistensi tipe data di DB
            db.execute(
                text("""
                    INSERT INTO geo_slope (
                        blok_id, objectid, kategori, kelerengan, luas, 
                        shape_leng, shape_area, geom_polygon, bulan, tahun
                    ) VALUES (
                        :bid, :obj_id, :kat, :kel, :luas, 
                        :sleng, :sarea, ST_Multi(ST_SetSRID(ST_GeomFromGeoJSON(:geom_json), 4326)), :b, :t
                    )
                """), 
                {
                    "bid": blok_id,
    ...
    if success_count == total_input and total_input > 0:
        ...
        if not last_error_msg:
            last_error_msg = f"{missing_blok_count} data kelerengan tertahan karena blok induk belum ada."
    else:
        final_status = "FAILED"
        if not last_error_msg:
            last_error_msg = "Seluruh data kelerengan gagal dimasukkan."
...
def get_geo_slope_by_period(db: Session, bulan: int, tahun: int, blok_id: str = None) -> list:
    """
    Menampilkan data geo_slope dalam bentuk list dictionary standar 
    termasuk melakukan konversi koordinat spasial MultiPolygon PostGIS ke format JSON.
    """
    query_str = """
        SELECT 
            id, id_slope, blok_id, objectid, kategori, kelerengan, luas,
            shape_leng, shape_area, ST_AsGeoJSON(geom_polygon) as geometry_json
        FROM geo_slope
